...
!!! example "نکات کلیدی برای آزمون"
      - تغییر مسیر STDOUT، STDERR و STDIN (`>`, `>>`, `2>`, `&>`, `<`، `<>`) و ترتیب آنها (مثلاً `2>&1`)
      - استفاده از `/dev/null` برای دور ریختن خروجی‌ها
      - لوله‌کردن دستورات با `|` و ترکیب ابزارها برای پردازش داده‌ها
      - استفاده از `xargs` برای تبدیل STDIN به آرگومان‌ها و `tee` برای ذخیره و نمایش همزمان خروجی
      - here-documents (`<<`) برای ارسال ورودی چند خطی به دستورات
 
 