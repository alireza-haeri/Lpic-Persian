...
!!! example "نکات کلیدی برای آزمون"
    - کنترل شغل‌ها: استفاده از `&`, `jobs`, `fg`, `bg` برای مدیریت پیش‌زمینه/پس‌زمینه
    - اجرای ماندگار با `nohup` و مدیریت خروجی آن (`nohup.out` یا redirect)
    - ارسال سیگنال‌ها با `kill`, `kill -SIGNAL PID`, `killall`, `pkill`
    - نظارت: `ps` (e.g., `ps -ef`), `top`, `pgrep`, `pkill`, `free`, `uptime`, `watch`
    - مولتی‌پلکسرهای ترمینال: کاربردهای پایه `screen` و `tmux` (detach/attach)
 
 